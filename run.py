...
class XAUUSDAnalyzer:
    ...
    def create_trading_prompt(self, tech_analysis, fundamental_context, current_price, timeframe):
        """Create optimized trading prompt for faster DeepSeek response"""
    ...
    def generate_fallback_analysis(self, df, current_price, timeframe):
        ...
        try:
            ...
            if bullish_signals >= 3 and bearish_signals <= 1:
                ...
                take_profit = current['close'] * 1.020
                risk_level = "MEDIUM"
                rr_ratio = "2.0"
            elif bearish_signals >= 3 and bullish_signals <= 1:
                recommendation = "SELL" 
                stop_loss = current['close'] * 1.015
                take_profit = current['close'] * 0.980
                risk_level = "MEDIUM"
                rr_ratio = "2.0"
            else:
                recommendation = "HOLD"
                stop_loss = current['close'] * 0.990
                take_profit = current['close'] * 1.010
                risk_level = "LOW"
                rr_ratio = "1.0"
...
• Stop loss ketat di ${stop_loss:.2f}
• Target profit ${take_profit:.2f}
• Monitor volume dan breakout levels

CATATAN:
Analisis ini menggunakan algoritma teknikal canggih. Untuk analisis fundamental mendalam, pastikan koneksi AI tersedia.
...
🎯 REKOMENDASI: HOLD
💰 ENTRY: ${current_price:.2f}
🛑 STOP LOSS: ${current_price * 0.99:.2f}
✅ TAKE PROFIT: ${current_price * 1.01:.2f}
...
if __name__ == '__main__':
    ...
    app.run(debug=True, port=5000, host='0.0.0.0')from flask import Flask, jsonify, request, render_template, send_from_directory
from flask_cors import CORS
import pandas as pd
import numpy as np
import requests
