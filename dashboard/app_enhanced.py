...
class EnhancedDataManager:
    ...
    def _generate_simple_data(self, pair: str, timeframe: str, days: int) -> pd.DataFrame:
        ...
        logger.info(f"Generated {len(prices)} synthetic data points for {pair}-{timeframe}")
...
class AdvancedRiskManager:
    ...
    def validate_trade(self, pair: str, signal: str, confidence: int, 
        ...
        if proposed_position_value > max_position_value:
            ...
            validation_result['warnings'].append(f"Position size reduced from {proposed_lot_size:.2f} to {validation_result['adjusted_lot_size']:.2f} lots")
        ...
        if validation_result['risk_score'] >= risk_threshold:
            validation_result['approved'] = False
            validation_result['rejection_reasons'].append(f"Overall risk score too high: {validation_result['risk_score']}/10")
...
def api_analyze():
    ...
    try:
        ...
        if price_data.empty:
            ...
            price_data = data_manager.get_price_data(pair, '1D', days=60)
        ...
        try:
            ...
            if not hist_df.empty:
                ...
                logger.info(f"Preparing {len(hist_df)} price series points for {pair}-{timeframe}")
                ...
                for _, row in hist_df.iterrows():
                    try:
                        ...
                        logger.warning(f"Error processing row in price series: {e}")
                        continue
        except Exception as e:
        ...
        logger.info(f"Analysis completed for {pair}-{timeframe}: {len(price_series)} chart points")
    ...
    try:
        ...
        return jsonify({
            ...
            'summary': {
                ...
                'final_balance': initial_balance + 1250.50,
...
def api_system_status():
    ...
    for timeframe in config.TIMEFRAMES:
        available_pairs = []
        for pair in config.FOREX_PAIRS:
            if (pair in data_manager.timeframe_data and 
                timeframe in data_manager.timeframe_data[pair] and
                not data_manager.timeframe_data[pair][timeframe].empty):
                available_pairs.append(pair)
        timeframe_availability[timeframe] = {
            'available_pairs': len(available_pairs),
            'total_pairs': len(config.FOREX_PAIRS)
        }
    
    return jsonify({
        'system': 'RUNNING',
        'historical_data': f"{len(data_manager.timeframe_data)} pairs loaded",
...
if __name__ == '__main__':
    logger.info("Starting Enhanced Forex Analysis System with Timeframe Fix...")
    logger.info(f"Supported pairs: {config.FOREX_PAIRS}")
